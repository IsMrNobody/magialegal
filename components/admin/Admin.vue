<template>
  <div>
    ...
    <v-expansion-panels>
        <v-expansion-panel
          v-for="(item,i) in ordenes"
          :key="i"        
          class="marron"
        >
        <v-expansion-panel-header v-if="item.active">
          <h3 :class="item.status === 'Pendiente' ? 'white--text' : 'accent--text'">{{ item.status }}
            <v-icon v-if="item.status === 'Pendiente'" :color="item.paid ? 'yellow' : ''">mdi-clock</v-icon>
            <v-icon v-if="item.status === 'Cancelado'" color="red">mdi-close</v-icon>
            <v-icon v-if="item.status === 'Entregado'" color="green">mdi-check</v-icon>
            <p>{{ item.createdAt }}</p>
          </h3>          
          <!-- <v-spacer></v-spacer> -->
          <!-- {{ item.createdAt }} -->
        ...
        <v-expansion-panel-content>
            <v-row>
                    ...
                    <v-list dense class="secondary">
                        ...
                        <v-list-item>
                          ...
                          <v-list-item-content class="display-1">
                            <div>
                              <v-icon v-if="item.paid" color="green">mdi-check</v-icon>
              ...
              <v-col>
                <v-card 
                  max-height="420"
                  class="overflow-auto secondary mt-5">
                    <v-col v-for="(pro, k) in item.products" :key="k">
                      <v-col>
                        <p>C/Product: <span class="yellow--text">{{ pro.cantidad }}</span></p>
                        <p class="grey--text">{{ pro.name }}</p>
                        <p>Portion: {{ pro.selectedPortion[0].title }}</p>
                        <p>Total : $ <span class="green--text">{{ pro.totalProduct }}</span></p>
                      </v-col>
                      <v-divider></v-divider>
                    </v-col>
                </v-card>
